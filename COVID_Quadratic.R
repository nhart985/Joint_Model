...
source("//Documents//Joint Model//Real_Functions_Quadratic.R")
...
ggsave(file="//Documents//Joint Model//Quadratic//SideBySide.pdf",width=18,height=8)
...
opt=optim(par=c(beta_raw_initial,alpha_raw_initial),fn=Test_Neg_Log_L,theta0=NA,theta1=NA,theta2=NA,surv_dat=surv_dat,raw=T,inf_dat=inf_dat,hessian=T)
...
opt=optim(par=c(beta_smooth_initial,alpha_smooth_initial),fn=Test_Neg_Log_L,theta0=theta0_smooth,theta1=theta1_smooth,theta2=theta2_smooth,surv_dat=surv_dat)
beta_smooth=opt$par[1:2]
alpha_smooth=opt$par[3]
temp_var=diag(solve(Test_D2_Log_L(beta_smooth,alpha_smooth,theta0_smooth,theta1_smooth,theta2_smooth,surv_dat=surv_dat,inf_dat=inf_dat)))
...
opt=optim(par=c(beta_stage1_initial,alpha_stage1_initial),fn=Test_Neg_Log_L,theta0=theta0_stage1,theta1=theta1_stage1,theta2=theta2_stage1,surv_dat=surv_dat)
beta_stage1=opt$par[1:2]
alpha_stage1=opt$par[3]
temp_var=diag(solve(Test_D2_Log_L(beta_stage1,alpha_stage1,theta0_stage1,theta1_stage1,theta2_stage1,surv_dat=surv_dat,inf_dat=inf_dat)))
stage1_ses=c(sqrt(temp_var),stage1_inf_ses)

opt=optim(par=c(beta_stage1_initial,alpha_stage1_initial,theta0_stage1,theta1_stage1,theta2_stage1,phi_stage1),fn=Neg_Log_L,surv_dat=surv_dat,inf_dat=inf_dat)
save(opt,file="//Documents//Joint Model//Quadratic//joint_opt.Rdata")
result=opt$par[1:6]
phi_joint=opt$par[7]
...
write.csv(alpha_result_table_naive,"//Documents//Joint Model//Quadratic//alpha_result_table_naive.csv",row.names=F)

alpha_ses_table_naive=matrix(c(naive_ses,period1_ses,period2_ses),nrow=3,ncol=3,byrow=T)
write.csv(alpha_ses_table_naive,"//Documents//Joint Model//Quadratic//alpha_ses_table_naive.csv",row.names=F)
...
write.csv(alpha_result_table,"//Documents//Joint Model//Quadratic//alpha_result_table.csv",row.names=F)

alpha_ses_table=matrix(c(raw_ses,smooth_ses,stage1_ses,joint_ses),nrow=4,ncol=6,byrow=T)
write.csv(alpha_ses_table,"//Documents//Joint Model//Quadratic//alpha_ses_table.csv",row.names=F)

Vcov_list=list(Vcov_smooth,Vcov_stage1,Vcov_joint)
save(Vcov_list,file="//Documents//Joint Model//Quadratic//Vcov_list.Rdata")

phi_list=list(phi_stage1,phi_joint)
save(phi_list,file="//Documents//Joint Model//Quadratic//phi_list.Rdata")

alpha_result_table_naive=read.csv("//Documents//Joint Model//Quadratic//alpha_result_table_naive.csv")
alpha_ses_table_naive=read.csv("//Documents//Joint Model//Quadratic//alpha_ses_table_naive.csv")
alpha_result_table=read.csv("//Documents//Joint Model//Quadratic//alpha_result_table.csv")
alpha_ses_table=read.csv("//Documents//Joint Model//Quadratic//alpha_ses_table.csv")
load("//Documents//Joint Model//Quadratic//Vcov_list.Rdata")
load("//Documents//Joint Model//Quadratic//phi_list.Rdata")
