...
h_t=function(beta,alpha,theta0,theta1,theta2,surv_dat,raw=F,inf_dat=NULL) {
...
Log_L=function(beta,alpha,theta0,theta1,theta2,phi,surv_dat,inf_dat) {
...
Neg_Log_L=function(par,surv_dat,inf_dat) {
  Z_names=names(surv_dat)[substr(names(surv_dat),1,1)=="Z"]
  result=-Log_L(par[1:length(Z_names)],par[length(Z_names)+1],par[length(Z_names)+2],par[length(Z_names)+3],par[length(Z_names)+4],par[length(Z_names)+5],surv_dat,inf_dat)
  show(c(par,result))
  return(result)
}

D2_Log_L=function(beta,alpha,theta0,theta1,theta2,phi,surv_dat,inf_dat) {
  ...
  hess_theta=matrix(c(dtheta0_theta0,dtheta0_theta1,dtheta0_theta2,
                      dtheta0_theta1,dtheta1_theta1,dtheta1_theta2,
                      dtheta0_theta2,dtheta1_theta2,dtheta2_theta2),nrow=3)
...
D2_Inf_Log_L=function(theta0,theta1,theta2,phi,inf_dat) {
...
Surv_Log_L=function(beta,alpha,theta0,theta1,theta2,surv_dat,raw=F,inf_dat=NULL) {
  haz_result=h_t(beta,alpha,theta0,theta1,theta2,surv_dat,raw,inf_dat)
  h_vals=haz_result$h
...
Inf_Log_L=function(theta0,theta1,theta2,phi,inf_dat,raw=F) {
  R=exp(theta0+theta1*inf_dat$s+theta2*inf_dat$s^2)
  l_I=sum(log(dnbinom(inf_dat$I,mu=R*inf_dat$I_prev,size=phi)))
  return(l_I)
}
